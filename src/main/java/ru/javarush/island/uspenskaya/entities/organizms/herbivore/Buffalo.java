...
@Setting(mass=700, speed= 3,maxQuality=10,saturation=100, icon="\uD83D\uDC03")
public class Buffalo extends Herbivore {
    private final String name;
    private final TreeMap<Integer,Class<?>> probabilityOfRation= new TreeMap();

    public Buffalo(double mass, int speed, int maxQuality, double saturation, String icon) {
        super(mass, speed, maxQuality, saturation, icon);
        AtomicInteger number = new AtomicInteger(1);
        this.name = "Buffalo"+ number.getAndIncrement();
    }
    ...
    public Organism clone() {
        return new Buffalo(this.getMass(), this.getSpeed(), this.getMaxQuality(), this.getSaturation(), this.getIcon());
    }
