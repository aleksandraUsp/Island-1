...
@Setting(mass=300, speed= 4,maxQuality=20,saturation=50, icon="\uD83D\uDC03")
public class Deer extends Herbivore {
    private final String name;
    private final TreeMap<Integer,Class<?>> probabilityOfRation= new TreeMap();

    public Deer(double mass, int speed, int maxQuality, double saturation, String icon) {
        super(mass, speed, maxQuality, saturation, icon);
        AtomicInteger number = new AtomicInteger(1);
        this.name = "Deer"+ number.getAndIncrement();
    }
