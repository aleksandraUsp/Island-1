...
@Setting(mass=1, speed= 4,maxQuality=200,saturation=0.15, icon="U+1F986")
public class Duck extends Herbivore {
    private final String name;
    private final TreeMap<Integer,Class<?>> probabilityOfRation= new TreeMap();

    public Duck(double mass, int speed, int maxQuality, double saturation, String icon) {
        super(mass, speed, maxQuality, saturation, icon);
        AtomicInteger number = new AtomicInteger(1);
        this.name = "Duck"+ number.getAndIncrement();
    }
    ...
    public Map<Integer,Class<?>> getProbabilityOfRation() {
        probabilityOfRation.put(90, Caterpillar.class);
    ...
    public Organism clone() {
        return new Duck(this.getMass(), this.getSpeed(), this.getMaxQuality(), this.getSaturation(), this.getIcon());
    }
