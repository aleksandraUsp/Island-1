...
@Setting(mass=60, speed= 3,maxQuality=140,saturation=10, icon="\uD83D\uDC10")
public class Goat extends Herbivore {
    private final String name;
    private final AtomicInteger number=new AtomicInteger(1);
    private final TreeMap<Integer,Class<?>> probabilityOfRation= new TreeMap();

    public Goat(double mass, int speed, int maxQuality, double saturation, String icon) {
        super(mass, speed, maxQuality, saturation, icon);
        this.name = "Goat"+number.getAndIncrement();
    }
    ...
    public Organism clone() {
        return new Goat(this.getMass(), this.getSpeed(), this.getMaxQuality(), this.getSaturation(), this.getIcon());
    }
